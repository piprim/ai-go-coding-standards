...
description: Design, implement, and refactor Ports & Adapters (Hexagonal) systems with clean domain boundaries, dependency inversion, and testable use-case orchestration across TypeScript, Java, Kotlin, and Go. Use this skill whenever the task involves new feature design, refactoring tightly-coupled services, separating domain logic from frameworks/persistence/transport, swapping infrastructure (DB, queue, external API), or supporting multiple delivery channels (HTTP / CLI / worker / cron) for the same use case — even if the user does not explicitly say "hexagonal", "ports and adapters", "DDD", or "clean architecture".
origin: ECC
---
